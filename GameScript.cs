// This script serves as the backbone of the game. It controls all of the functionality within the "_main" scene.
// Upon initiation (scene load) a countdown is initiated "Counting()" with a corresponding UI countdown on screen.
// At the end of the countdown the script picks a random mole (actor type) and moves it up and down, using raycasting to check if
// the user taps/clicks the mole before it returns to its original downright position. If tapped, chooses another mole and repeats
// If not tapped, the game ends and the score is passed as a player preference (global variable) to the "_end" scene to be displayed
